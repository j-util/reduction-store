...
For contributors, `./mvnw verify` also builds the isolated Maven reactor under
`reduction-store-processor/src/it`. Its downstream consumer compiles against
separately compiled model and reduction artifacts and exercises the generated
store at runtime, independently of the processor test harness.
...
cross-module use, the input type, reduction classes and their no-argument
constructors, and object-state types must be accessible from the definition
package. In practice, types supplied by another module normally need to be
public, as do the required constructors and enclosing types.
...
- For automatic discovery, `P` must be a top-level, non-generic type compiled
  ...
  instead reference an accessible input and reductions from compiled
  dependencies.
...
- Automatic discovery covers source roots in the current full compilation. It
  ...
  a later round. Explicit definitions resolve only their listed class values
  through the compiler type model and retry unresolved generated types in later
  rounds; they do not scan dependencies or broaden discovery. Incremental or
  IDE partial compilation cannot reliably reconstruct an automatic aggregate;
  use a clean/full compile after adding, removing, moving, or renaming a
  reduction.
